...
public class TokenService {
    ...
    public String generateToken(Usuarios usuarios) {
        try {
            ...
            String token = JWT
                    .create()
                    .withIssuer("auth-api")
                    .withSubject(usuarios.getEmailInstitucional())
    ...
    public String validationToke(String token){
        try{
            Algorithm algorithm = Algorithm.HMAC256(secret);
            return JWT
                    .require(algorithm)
            ...
            throw new RuntimeException("erro ao validar token" + exception.getMessage(),exception);
        }
    }

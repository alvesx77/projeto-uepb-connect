...
async function verificarDisponibilidade(campo, valor) {
    const response = await fetch(`${API_URL}/validacao/${campo}?valor=${encodeURIComponent(valor)}`);
    if (response.ok) {
...
btn.addEventListener("click", async () => {
    ...
    if (Imatricula.value.trim() !== "") {
        const resultadoMatricula = await verificarDisponibilidade("matricula", Imatricula.value.trim());
    ...
    dadosTela.matricula = Imatricula.value;
    dadosTela.situacaoEmpregabilidade = situacaoEmpregabilidade;
    dadosTela.curriculo = Icurriculo.value;
